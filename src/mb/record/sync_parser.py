...
class ParserNZSM(BaseParserNZSM):
    ...
    def parse_archive(file_path: str | IO[bytes], user_id: UUID, file_name: str | None = None) -> list[str]:
        ...
        _populate_common_fields(ParserNZSM.parse_file(lines[:num_lines]))
        _populate_common_fields(ParserNZSM.parse_file(lines[num_lines : 2 * num_lines]))
        _populate_common_fields(ParserNZSM.parse_file(lines[2 * num_lines :]))
