...
Along with other long-running tasks such as parsing raw data, it is offloaded to `taskiq` workers.
The communication among `FastAPI`, `MongoDB` and `taskiq` workers is coordinated by `RabbitMQ`.
